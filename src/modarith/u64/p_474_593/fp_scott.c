...
// Command line : python monty.py 64 593 * 2**474 - 1
...
static spint inline flatten(spint *n) {
  spint carry = prop(n);
  n[0] -= (spint)1u & carry;
  n[7] += ((spint)0x128800000000000u) & carry;
  (void)prop(n);
  return (carry & 1);
}
...
static void inline modadd(const spint *a, const spint *b, spint *n) {
  ...
  n[7] = a[7] + b[7];
  n[0] += (spint)2u;
  n[7] -= (spint)0x251000000000000u;
  carry = prop(n);
  n[0] -= (spint)2u & carry;
  n[7] += ((spint)0x251000000000000u) & carry;
  (void)prop(n);
}
...
// maximum possible = 42727735367436755092501244560835346440
// Modular multiplication, c=a*b mod 2p
static void inline modmul(const spint *a, const spint *b, spint *c) {
  dpint t = 0;
  spint p7 = 0x128800000000000u;
...
static void nres(const spint *m, spint *n) {
  const spint c[8] = {0x1c01ba10679bdb43u, 0x8bdf30c84f6ef29u,
                      0x19e6f61221ac3feu,  0x1213dbbca7803742u,
                      0x4886b0ff917be61u,  0x129e00dd0833cdecu,
                      0x1fe45ef986427b77u, 0x1074f37b0910d61u};
  modmul(m, c, n);
}
...
static void inline modmli(const spint *a, int b, spint *c) {
  ...
  spint mask = ((spint)1 << 61u) - (spint)1;
  dpint t = 0;
  spint q, h, r = 0x37420cf37b0910d6;
  t += (dpint)a[0] * (dpint)b;
  c[0] = (spint)t & mask;
  t = t >> 61u;
  t += (dpint)a[1] * (dpint)b;
